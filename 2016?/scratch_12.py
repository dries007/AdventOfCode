...
lineRe = re.compile(r'((?:\w+-)+)(\d+)\[(\w+)\]')

total = 0

for i in input.split("\n"):
    match = lineRe.match(i)
    name = match.group(1)
    ...
    for c in sorted(d, key=d.get, reverse=True):
        count = d[c]
        if count in d2:
            d2[count] = ''.join(sorted(d2[count] + c))
        else:
            d2[count] = c
    key = "".join(d2.values())[:5]
        ...
        for _ in range(sector):
            ...
            for c in tmp:
                if c == '-' or c == ' ':
                    ...
                    dec += chr(ord(c) + 1)
            tmp = dec
        print(name, '\t', dec, sector)

    # print(name, sector, check, d2, key, key == check)
