...
class Bot:
    ...
    #             self._high = value
    #     else:
    #         raise RuntimeError("Bot only has 2 slots")
    #     self.run()

    def __repr__(self):
        return "<Bot #%s L:%s H:%s>" % (self.id, self._low, self._high)

    def routes(self, botL, valueL, botH, valueH):
        if self._routes:
            raise RuntimeError("Routes already set")
        self._routes = ((botL, valueL), (botH, valueH))
        self.run()

    def run(self):
        if self._high and self._low and self._routes:
            routeL, routeH = self._routes
            outL = bots if routeL[0] else outputs
            outH = bots if routeH[0] else outputs
            outL[routeL[1]].append(self._low)
            self._low = None
            outH[routeH[1]].append(self._high)
            self._high = None
    ...
    def append(self, value):
        ...
        if value < self._low:
            ...
            self._high = value
        if self._low in targets and self._high in targets:
            print('Bot #%s compares %s and %s' % (self.id, self._low, self._high))
        if self._routes:
            self.run()
...
def process(line):
    ...
    if match:
        bots[int(match.group(2))].append(int(match.group(1)))
        return
    match = re.search(r'bot (\d+) gives low to (output|bot) (\d+) and high to (output|bot) (\d+)', line)
    if match:
        bot = int(match.group(1))
        print(bot, bots[bot], match.groups())
        bots[bot].routes(match.group(2) == 'bot', int(match.group(3)), match.group(4) == 'bot', int(match.group(5)))
        return
    raise RuntimeError("Format error: %s" % line)
