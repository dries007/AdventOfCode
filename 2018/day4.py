...
RE_TIMESTAMP = re.compile(r'\[(\d+)-(\d+)-(\d+) (\d+):(\d+)]', re.IGNORECASE)
RE_BEGIN = re.compile(r'\[\d+-\d+-\d+ \d+:\d+\] Guard #(\d+) begins shift', re.IGNORECASE)
...
def day4(inp):
    ...
    for line in inp.splitlines():
        y, m, d, hh, mm = map(int, RE_TIMESTAMP.match(line).groups())
        o = datetime.datetime(year=y, month=m, day=d, hour=hh, minute=mm).timestamp()
        lines.append((o, line))
    ...
    for _, line in lines:
        ...
        if match:
            ...
            # print('Wakes')
            # print(guard, dict(guards[guard]))
            # if sleeptime_hh == 23:
            #     for i in range(sleeptime_mm, 60):
            #         guards[guard][i] += 1
            #     for i in range(0, mm):
            #         guards[guard][i] += 1
            # else:
            for i in range(sleeptime_mm, mm):
                guards[guard][i] += 1
        else:
            raise RuntimeError("Input bad")
        # print(y, m, d, hh, mm)

    # Part 1
    sums = sorted({i: sum(v) for i, v in guards.items()}.items(), key=lambda x: x[1], reverse=True)
    guard_id_1 = sums[0][0]
    for i, guard in guards.items():
        print(i, '->', *sorted(guard.items(), key=lambda x: x[1], reverse=True))
    guard = guards[guard_id_1]
    print('MAX ->', guard_id_1, *sorted(guard.items(), key=lambda x: x[1], reverse=True))
    max_min_1, _ = max(guard.items(), key=lambda x: x[1])
    print("START 1: Guard", guard_id_1, "Mins", max_min_1)

    # Part 2
    p2list = [(
        i, [*sorted(v.items(), key=lambda x: x[1], reverse=True)]
    ) for i, v in guards.items()]
    p2list.sort(key=lambda x: x[1][0][1], reverse=True)

    # print(*p2list, sep='\n')

    return guard_id_1 * max_min_1, p2list[0][0] * p2list[0][1][0][0]
